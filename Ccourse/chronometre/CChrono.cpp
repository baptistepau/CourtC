...
bool CChrono::tours()
{
	...
	if (etat == true)
	{
		...
		clockStop = clock();
		nbTours += 1;
		tempsTours = (clockStop - clockStart) / CLOCKS_PER_SEC;
		if ((tempsTours <= meilleurTours)||(meilleurTours == 0) ) 
		{
			meilleurTours = tempsTours;
		}
		clockStart = clock();
		return true;
	}
...
int CChrono::getMeilleurTemps()
{
	return meilleurTours;
}
int CChrono::getnbTours()
{
	return nbTours;
}